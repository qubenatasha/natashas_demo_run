...
class natashas_demo_runService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = natashas_demo_run.query.get(entity_id)
        if data is None:
            raise natashas_demo_runServiceError(
                'natashas_demo_run {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = natashas_demo_run.query.filter(
            natashas_demo_run.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = natashas_demo_run.query.get(entity_id)  # natashas_demo_run is a mongo class
        if record is None:
            raise natashas_demo_runServiceError(
                'natashas_demo_run {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
